...
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <ItemGroup>
    ...
    <!-- 示例配置文件 -->
    <None Include="resources\config.example.yaml">
      <CopyToOutputDirectory>Always</CopyToOutputDirectory>
      <Link>config.example.yaml</Link>
    </None>
  </ItemGroup>
...
创建 `src/Models/CheckinConfig.cs`：
...
创建 `src/Models/PlayerCheckinData.cs`：
...
创建 `src/Services/CheckinService.cs`：
...
创建 `src/Commands/CheckinCommand.cs`：
...
创建 `src/Commands/CheckinInfoCommand.cs`：
...
创建 `src/Commands/CheckinTopCommand.cs`：
...
创建 `src/Events/PlayerEventHandler.cs`：
...
在 `resources/` 目录下创建 `config.example.yaml`：
...
编译后，`bin/Debug/net9.0/` 目录包含：

```
bin/Debug/net9.0/
├── CheckinPlugin.dll           # 插件主 DLL
├── CheckinPlugin.pdb           # 调试符号
├── plugin.json                 # 插件元数据（从 resources/ 复制）
└── config.example.yaml         # 示例配置（从 resources/ 复制）
```

**注意**：资源文件通过 `.csproj` 中的 `<Link>` 标签配置，会自动从 `resources/` 目录复制到输出目录的根目录。
